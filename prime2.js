...
//canvas and context are (constant) variables initialized to draw to the canvas
...
//checkKey speeds up the rate at which pending_changes are applied if right arrow is pressed, and slows down the rate if left arrow is pressed
function checkKey(e) {
  e = e || window.event;
...
// display_changes applies the first element of pending_changes and sets a timeout between changes
...
// (2) initializing gdata.colors to all green except the first element (since 1 is not prime)
// (3) calculating the height needed for the canvas
// (4) calling drawStuff to create the grid, starting the sieve, and setting the initial timeout period before pending_updates are applied
...
//changes the color of n by calculating the position of n in the grid and adding it to pending_updates
...
//checks for the next smallest number that has not been proven composite (i.e., marked green)
...
//marks primes as orange and composites as red. Composites are marked red by iterating through multiples of primes (sieve of Eratosthenes)
...
//this function allows for generalization to a simulation for multiple sieves. It is not important to this code and may be ignored.
function start() {
	start_sieve1();
}

//this function implements the sieve by repeatedly getting the next prime and marking its multiples as composite through get_next_prime() and iterate().
